 ...
 *
 * @template-extends ExceptionHandler<Request, Response>
 * @package   Rebet
    ...
    public function report($input, $result, \Throwable $e) : void
    {
        ...
        switch (HttpStatus::classOf($status)) {
            ...
            case HttpStatus::CLIENT_ERROR:
                ...
                Log::debug("HTTP {$status} {$reason} occurred.", compact('input'), $e);
                return;

            case HttpStatus::SERVER_ERROR:
                $reason = HttpStatus::reasonPhraseOf($status) ?? 'Unknown Server Error' ;
                Log::error("HTTP {$status} {$reason} occurred.", compact('input'), $e);
                return;
