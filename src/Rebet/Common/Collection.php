 ...
 * Collection Class
 *
 * This class based on Illuminate\Support\Collection of laravel/framework ver 5.7.
 *
 * Function diffs between Laravel and Rebet are like below;
 *  - remove Illuminate modules dependency. (Arr, Macroable, Jsonable and Arrayable)
 *  - remove Symfony modules dependency. (VarDumper)
 *  - remove functions dump and dd.
 *  + supported: Describable trait
 *  + supported: Convertible interface
 *
...
class Collection implements ArrayAccess, Countable, IteratorAggregate, JsonSerializable, Convertible
{
    ...
    {
        if ($callback) {
            return new static(Arrays::where($this->items, $callback));
        }
     ...
     */
    public function random($number = 1) : self
    {
        [$selected, ] = Arrays::random($this->items, $number);
    ...
    {
        return new static(Arrays::shuffle($this->items, $seed));
    }
