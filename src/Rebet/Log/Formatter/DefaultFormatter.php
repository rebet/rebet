...
class DefaultFormatter implements LogFormatter {
   ...
   /**
	 ...
	 */
	public function format(DateTime $now, int $level, $message, array $context = [], $error = null, array $extra = []) {
		$body = '';

		if(!is_string($message) && !method_exists($message, '__toString')) { $message = print_r($message, true); }
		$body = $now->format('Y-m-d H:i:s.u')." ".getmypid()." [".self::$LOG_LEVEL_LABEL[$level]."] ".$message;
