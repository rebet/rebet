...
class BuiltinRansackerTest extends RebetDatabaseTestCase
{
    ...
    {
        $this->eachDb(function (Database $db) {
            $this->assertEquals(['name = :name', ['name' => 'foo']], BuiltinRansacker::of($db)->resolve('name', 'foo'));
        });
    }
    ...
    public function dataBuilds() : array
    {
        return [
            ...
            [
                ['name = :name AND gender = :gender', ['name' => 'foo', 'gender' => Gender::MALE()]],
            ...
            [
                ['name = :name AND resign_at IS NOT NULL', ['name' => 'foo']],
